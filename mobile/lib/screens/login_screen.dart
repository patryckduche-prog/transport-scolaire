...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> loadAlerts() async {
    try {
      ...
      setState(() {
        alerts = data;
        seasonal = SeasonalTheme.fromDate(DateTime.now(), weather: SeasonalTheme.weatherFromAlerts(data));
        alertsLoading = false;
      });
    } catch (_) {
      if (!mounted) return;
      setState(() {
        ...
        seasonal = SeasonalTheme.fromDate(DateTime.now(), weather: WeatherState.fog);
        alertsLoading = false;
      });
    }
  }
  ...
  Future<void> openPassengerAccess() async {
    ...
    try {
      ...
      final data = await api.startPassengerAccess();
      ...
      Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (_) => const PassengerDashboardScreen()));
    } catch (_) {
      setState(() => error = 'Acces parent / eleve indisponible. Verifiez le serveur.');
    } finally {
  ...
  Future<void> submitDriverCode() async {
    ...
    try {
      ...
      setState(() => error = 'Code conducteur invalide ou serveur indisponible.');
    } finally {
      if (mounted) setState(() => loading = false);
    }
  }
  ...
  Future<void> submitAdminLogin() async {
    ...
    try {
      ...
      Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (_) => _homeFor(user.role)));
    } catch (_) {
    ...
    showModalBottomSheet<void>(
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            Text('Acces entreprise / region', style: Theme.of(context).textTheme.titleLarge),
            const SizedBox(height: 14),
            TextField(controller: adminEmail, decoration: const InputDecoration(labelText: 'Email', prefixIcon: Icon(Icons.mail_outline))),
            const SizedBox(height: 12),
            TextField(controller: adminPassword, obscureText: true, decoration: const InputDecoration(labelText: 'Mot de passe', prefixIcon: Icon(Icons.lock_outline))),
            const SizedBox(height: 18),
            FilledButton.icon(onPressed: loading ? null : submitAdminLogin, icon: const Icon(Icons.admin_panel_settings_outlined), label: const Text('Ouvrir la gestion')),
          ],
        ),
      ),
    );
        ...
        child: RefreshIndicator(
          ...
          child: ListView(
            ...
            children: [
              ...
              _AlertsPanel(alerts: alerts, loading: alertsLoading, theme: seasonal),
              const SizedBox(height: 18),
              FilledButton.icon(
              ...
              const SizedBox(height: 18),
              Text('Conducteur', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
              const SizedBox(height: 8),
              TextField(
                controller: driverCode,
                textCapitalization: TextCapitalization.characters,
                decoration: const InputDecoration(labelText: 'Code conducteur', prefixIcon: Icon(Icons.pin_outlined)),
              ),
              if (error != null)
              ...
              OutlinedButton.icon(
                ...
                label: Text(loading ? 'Ouverture...' : 'Ouvrir espace conducteur'),
              ),
              const SizedBox(height: 8),
              TextButton.icon(
        ...
        children: [
          ...
          DecoratedBox(
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: [Colors.black.withValues(alpha: 0.62), Colors.black.withValues(alpha: 0.1)],
              ),
            ),
          ),
          const Positioned(
          ...
          Positioned(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(color: theme.primary.withValues(alpha: 0.86), borderRadius: BorderRadius.circular(8)),
              child: Row(
                children: [
                  Icon(theme.alertIcon, color: Colors.white, size: 18),
                  const SizedBox(width: 8),
                  Text(theme.label, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
                ],
              ),
            ),
          ),
...
class _AlertsPanel extends StatelessWidget {
  const _AlertsPanel({required this.alerts, required this.loading, required this.theme});
          ...
          children: [
            Row(
              children: [
                ...
                const SizedBox(width: 8),
                Text('Alertes en cours', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
              ],
            ),
              ...
              ...alerts.take(3).map((item) {
                ...
                return Padding(
                  ...
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Icon(warning ? theme.alertIcon : Icons.check_circle_outline, size: 20, color: warning ? Colors.orange.shade800 : Colors.green.shade700),
                      const SizedBox(width: 8),
                      Expanded(child: Text(alert['message'] as String? ?? 'Aucune information disponible.')),
                    ],
                  ),
                );
              }),
